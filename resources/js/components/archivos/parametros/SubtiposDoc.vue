                ...
                <table class="table table-hover table-striped table-responsive" id="divTable">
                    ...
                    <tbody>
                        <tr v-for="(r, index) in registros">
                            <td width="3%" scope="row">{{ r.stdoc_id }}</td>
                            ...
                            <td>
                                <span v-if="r.stdoc_estado === 'A'" class="badge badge-success">ACTIVO</span>
                                <span v-else-if="r.stdoc_estado !== 'A'" class="badge badge-warning">{{ r.stdoc_estado
                                    }}</span>
                            </td>
                        </tr>
                    </tbody>
                    ...
                    <div class="modal-body">
                        <div class="row justify-content-left">
                            <div class="col-md-12">
                                ...
                                <select v-model="registro.stdoc_tdoc_id" class="form-control"
                                    placeholder="Tipo de Documento" disabled>
                                    <option value="null">-- Seleccione Tipo de Documento --</option>
                                    <option v-for="s in tiposDoc" v-bind:value="s.tdoc_id">
                                        {{ s.tdoc_descripcion}} - {{ s.tdoc_codigo}}
                                    </option>
                                </select>
                            </div>
                            <div class="col-md-6">
                                <label>Codigo</label>
                                <input v-model="registro.stdoc_codigo" class="form-control" placeholder="Codigo"
                                    disabled />
                            </div>
                            <div class="col-md-6">
                                <label>Descripcion</label>
                                <input v-model="registro.stdoc_descripcion" class="form-control"
                                    placeholder="Descripcion" disabled />
                            </div>
                            <div class="col-md-12">
                                <label>Contenido</label>
                                <textarea v-model="registro.stdoc_contenido" class="form-control"
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                plural: "Subtipos Documentales",
                singular: "Subtipo Documental",
                seleccionado: "",
                errores: [],
                registro: {},
                registros: [],
        ...
        methods: {
            listarRegistros() {
                let url = "api/subtiposDoc";
                axios.get(url).then((response) => {
                    this.registros = response.data.data; //twice data
                    console.log("Subtipos: ", this.registros);
                });
            },
            ...
            listarTiposDoc() {
				...
				axios.get(url).then(response => {
					this.tiposDoc = response.data.data; //twice data
				});
			},
            ...
            insertarRegistro(e) {
                this.errores = [];
                /*
                                    if (!this.registro.trb_data.stdoc_codigo) {
                                        this.errores.push('Falta CI');
                                    }
                                    if (!this.registro.trb_data.stdoc_descripcion || !this.registro.trb_data.trb_materno) {
            ...
            actualizarRegistro(e) {
                ...
                if (this.errores.length === 0) {
                    ...
                    //gRegistro.stdoc_usr_id = 1;
                    let that = this;
                    let url = "api/subtiposDoc/" + gRegistro.stdoc_id;
                    axios
                        .put(url, gRegistro)
                        .then(function (response) {
                            that.output = response.data;
