                ...
                <table class="table table-hover table-striped table-responsive" id="divTable">
                    ...
                    <tbody>
                        <tr v-for="(r, index) in registros">
                            <td width="3%" scope="row">{{ r.arch_id }}</td>
                            ...
                            <td>
                                <span v-if="r.arch_estado === 'A'" class="badge badge-success">ACTIVO</span>
                                <span v-else-if="r.arch_estado !== 'A'" class="badge badge-warning">{{ r.arch_estado
                                    }}</span>
                            </td>
                        </tr>
                    </tbody>
                    ...
                    <div class="modal-body">
                        <div class="row justify-content-left">
                            <div class="col-md-6">
                                ...
                                <select v-model="registro.arch_tarch_id" class="form-control"
                                    placeholder="Tipo de Archivo" disabled>
                                    <option value="null">-- Seleccione Tipo de Archivo --</option>
                                    <option v-for="s in tiposArchivo" v-bind:value="s.tarch_id">
                                        {{ s.tarch_descripcion}} - {{ s.tarch_codigo}}
                                    </option>
                                </select>
                            </div>
                            <div class="col-md-6">
                                <label>Código</label>
                                <input v-model="registro.arch_codigo" class="form-control" placeholder="Codigo"
                                    disabled />
                            </div>
                            <div class="col-md-6">
                                <label>Descripción</label>
                                <input v-model="registro.arch_descripcion" class="form-control"
                                    placeholder="Descripcion" disabled />
                            </div>
                            <div class="col-md-6">
                                <label>Archivo Padre</label>
                                <select v-model="registro.arch_padre_id" class="form-control" placeholder="Padre"
                                    disabled>
                                    <option value="-1">-- Seleccione Archivo Padre --</option>
                                    <option v-for="s in archivosPadres" v-bind:value="s.arch_id">
                                        {{ s.arch_descripcion }} - {{ s.arch_codigo }}
                                    </option>
                                </select>
                            </div>
                        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            listarRegistros() {
                let url = "api/archivos";
                axios.get(url).then((response) => {
                    this.registros = response.data.data; //twice data
                    this.archivosPadres = this.registros;
                    console.log("Archivos: ", this.registros);
                });
            },
            ...
            listarTiposArchivo() {
				...
				axios.get(url).then(response => {
					this.tiposArchivo = response.data.data; //twice data
				});
			},
            ...
            insertarRegistro(e) {
                this.errores = [];
                /*
                                              if (!this.registro.trb_data.arch_codigo) {
                                                  this.errores.push('Falta CI');
                                              }
                                              if (!this.registro.trb_data.arch_descripcion || !this.registro.trb_data.trb_materno) {
            ...
            actualizarRegistro(e) {
                ...
                if (this.errores.length === 0) {
                    ...
                    //gRegistro.arch_usr_id = 1;
                    let that = this;
                    let url = "api/archivos/" + gRegistro.arch_id;
                    axios
                        .put(url, gRegistro)
                        .then(function (response) {
                            that.output = response.data;
